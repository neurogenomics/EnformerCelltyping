...
#Use split, pretrained model and just use DNA output (ignore cell typing side)
model = Enformer_Celltyping(
                            use_prebuilt_model=True,
                            model_arch = 'split',
                            # Split model, i.e. from pretraining step
...
pred_bigwigs = {i:pyBigWig.open(out_pth+f"{cell}_DNA_only_{i}.bigWig", "w") for i in hist_marks}
    ...
    while (strt+(WINDOW_SIZE_DNA+buff_ca_dna)<=end):    
        ...
        if batch_count==batch_size:
            ...
            pred = model.predict({"dna":multi_X_dna,
                                 ...
                                 },return_avg = True) #only average side of split model
            #store res
